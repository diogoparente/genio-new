...
export function LanguageSwitcher({ currentLanguage = "en" }: { currentLanguage?: string }) {
	...
	return (
		<select
			...
			className="appearance-none cursor-pointer text-sm font-medium pl-2.5 pr-7 py-1.5 rounded-[var(--radius-neu-full)] bg-[var(--color-neu-surface)] text-[var(--color-neu-text-primary)] border border-[var(--neu-shadow-dark)]/20 shadow-neu-inset-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-neu-accent)]/50 transition-shadow hover:shadow-neu-sm bg-no-repeat"
			style={{
